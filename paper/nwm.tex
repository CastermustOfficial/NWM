...
rejects dangerous actions before maximizing reward. We formalize the method and
evaluate it against three baselines---a uniform random policy, tabular
Q-learning with state aggregation, and a Deep Q-Network (DQN)---on three
classic-control benchmarks spanning a difficulty gradient (CartPole, Acrobot,
MountainCar), reporting mean and standard deviation over five seeds under a
fixed evaluation protocol. \NWM{} is the strongest method on the dense,
low-dimensional CartPole task, learns but trails DQN on Acrobot, and---like the
other non-bootstrapping baselines---fails on sparse-reward MountainCar. The
study thus characterizes precisely where an instance-based potential-field
method is competitive and where parametric value learning retains an advantage.
All code, seeds, and figures are released to make the study fully reproducible.
\end{abstract}
...
\begin{enumerate}
  ...
  mechanisms that were previously only described informally.
...
\State Roll out the episode: at each step select an action by
$\varepsilon$-greedy over the Fear \& Greed rule on Eq.~\eqref{eq:force};
buffer $(\tilde x_t, a_t, r_t)$.
...
\emph{On the dense, low-dimensional task, \NWM{} is the strongest method.} On
CartPole-v1 it attains the best final greedy evaluation,
$159.8\pm94.0$, ahead of DQN ($100.8\pm30.3$) and tabular Q-learning
($98.4\pm20.2$), and far above the random policy ($25.1$). This is consistent
with the method's design: when the observation geometry is smooth and a good
action generalizes across nearby states, aggregating attractive/repulsive forces
from neighbouring prototypes is an effective, low-bias estimator, and the
percentile scoring lets the agent exploit good episodes immediately. The trade
-off is variance---\NWM{}'s final-eval standard deviation and its lower AUC
($83.1$ vs.\ $108.7$--$115.3$) show a noisier, later-peaking learning curve than
the bootstrapped baselines (Figure~\ref{fig:curves}).

\emph{On the medium task, \NWM{} learns but trails DQN.} On Acrobot-v1 \NWM{}
reaches $-307.9$, clearly better than tabular Q ($-423.1$) and Random ($-499.9$)
but below DQN ($-165.8$), which nearly solves the task (best seed $-77.8$
...
percentile score only approximates.

\emph{On the sparse-reward task, only bootstrapped value learning succeeds.} On
MountainCar-v0 every method except DQN remains pinned at the $-200$ truncation
floor: without shaping, \NWM{}, tabular Q, and the random policy never
experience the goal often enough for the field (or the table) to form, whereas
DQN's bootstrapped backups propagate the single terminal reward and reach
$-174.6$. This is the clearest illustration of \NWM{}'s central limitation
(Section~\ref{sec:limitations}): it has no bootstrapping mechanism to
manufacture a learning signal where reward is absent.
...
weighting) rather than a bootstrapped Bellman backup, which limits performance
on long-horizon, sparse-reward tasks. Third, the force model and its constants
(thresholds, decays, lock criteria) are hand-designed; while robust across the
tested tasks, they are not learned. We view \NWM{} as a transparent,
reproducible baseline for non-parametric control rather than a state-of-the-art
method, and a useful component for hybrid systems.
