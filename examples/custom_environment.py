...
def main():
    ...
    config = NWMConfig(
        max_centroids=200, warmup_episodes=20, exploration_rate=1.0, min_exploration=0.1
    )

    agent = NWM(
        state_dim=env.observation_space.shape[0], num_actions=env.action_space.n, config=config
    )
