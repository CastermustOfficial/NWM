...
class TestPersistentCentroid:
    ...
    def test_get_force_attraction(self):
        ...
        centroid = PersistentCentroid(state=np.zeros(4), score=0.9, action=0)
    ...
    def test_get_force_repulsion(self):
        ...
        centroid = PersistentCentroid(state=np.zeros(4), score=0.1, action=0)
        force = centroid.get_force(0)
        assert force < 0  # Repulsion

    def test_get_force_unknown_action(self):
        """Test that unknown actions return zero force."""
        centroid = PersistentCentroid(state=np.zeros(4), score=0.8, action=0)
        assert centroid.get_force(99) == 0.0

    def test_action_variance(self):
        """Test variance calculation."""
        centroid = PersistentCentroid(state=np.zeros(4), score=0.5, action=0)
        # Add varied scores
    ...
    def test_repr(self):
        ...
        centroid = PersistentCentroid(state=np.zeros(4), score=0.8, action=1)
